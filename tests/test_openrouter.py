...
def test_cv_response():
    provider, api_key = get_test_api_key("openrouter")
    if not api_key:
        print(f"[SKIP] No active openrouter API key found in DB")
        return None

    print(f"[TEST] openrouter CV response — using key: {api_key['name']}")
    try:
        resp = openrouter.call(api_key["apiKey"], SYSTEM_MSG, USER_MSG, model_name=provider.get("model_name"))
...
def test_message_response():
    ...
    print(f"[TEST] openrouter Message response — using key: {api_key['name']}")
    try:
        resp = openrouter.call(api_key["apiKey"], SYSTEM_MSG_MSG, USER_MSG_MSG, model_name=provider.get("model_name"))
        print(f"[OK] API call succeeded")
