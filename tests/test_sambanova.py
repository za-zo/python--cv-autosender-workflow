...
def test_cv_response():
    provider, api_key = get_test_api_key("sambanova")
    if not api_key:
        print(f"[SKIP] No active sambanova API key found in DB")
        return None

    print(f"[TEST] sambanova CV response — using key: {api_key['name']}")
    try:
        resp = sambanova.call(api_key["apiKey"], SYSTEM_MSG, USER_MSG, model_name=provider.get("model_name"))
...
def test_message_response():
    ...
    print(f"[TEST] sambanova Message response — using key: {api_key['name']}")
    try:
        resp = sambanova.call(api_key["apiKey"], SYSTEM_MSG_MSG, USER_MSG_MSG, model_name=provider.get("model_name"))
        print(f"[OK] API call succeeded")
...
if __name__ == "__main__":
    print(f"\n{'='*50}")
    print(f"  Testing sambanova")
    print(f"{'='*50}")
    r1 = test_cv_response()
    r2 = test_message_response()
    if r1 is None and r2 is None:
        print(f"\n⏭️  sambanova — SKIPPED (no API key)")
